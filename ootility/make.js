...
Edit Files
----------
1. support/index.html                    Link to each usage example
...
//// 1. support/index.html                    Link to each usage example
updateSupportIndex('support/index.html', 'support')
...
////
function updateSupportIndex (htmlPath, supportPath) {
...
`  //// This dynamic section is kept up to date by ‘ootility/make.js’ /////// -->`])
    demos.forEach( name => {
        ...
        if ( 0 > file[i].indexOf('Redirecting...') )
            out.push(`
  <a href="${name}">
